#!/usr/bin/env python3
"""Plot raw T4 w1/160-ms PC repetitions by cell and raw position."""
...
OUTPUT = Path(__file__).with_name("t4_160ms_w1_pc_raw_positions_by_cell.png")
...
def _values(h5, dataset, cell_index):
    return np.atleast_1d(np.asarray(_object(h5, dataset[cell_index, 0])).squeeze())


def _condition_repetitions(h5, result, index):
    if float(np.asarray(_object(h5, result["empty"][index])).squeeze()):
        return None
    data = _object(h5, result["data"][index])
    traces = []
    for reference in np.asarray(data["align"]["rep"]["data"]).ravel():
        recording = np.asarray(h5[reference], dtype=float)
        trace = np.interp(TIMES_MS, recording[0], recording[1])
        trace -= trace[(TIMES_MS >= -200.0) & (TIMES_MS < 0.0)].mean()
        traces.append(trace)
...
def main():
    ...
    with h5py.File(RAW_MAT, "r") as h5:
        ...
        for cell_index in range(16):
            ...
            for position in POSITIONS:
                hits = np.where(np.isclose(positions, position))[0]
                if not hits.size:
                    continue
                index = (pc_index, width_index, duration_index, int(hits[0]))
                traces = _condition_repetitions(h5, result, index)
                if traces is not None:
                    cell_traces[position] = traces
    ...
    for row, cell in enumerate(range(1, 17)):
        for col, position in enumerate(POSITIONS):
            ...
            if row == 0:
                ax.set_title(f"raw pos {position:+d}", fontsize=7, pad=3)
            if col == 0:
    ...
    fig.suptitle(
        ...
        "columns = raw stimulus position; gray = recordings, green = mean; "
