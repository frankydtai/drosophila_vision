...
def plot_fris(
    ...
    **kwargs,
    ...
    if fris.shape[1] == 1:
        # Rows must be (n_models, n_cell_types). `fris.T.squeeze()` collapses to 1D when
        # n_models==1 (shape (N, 1, 1)), which breaks scatter_on_violins_with_best.
        x = np.asarray(fris)
        n_t, n_m = x.shape[0], x.shape[1]
        x2 = x.reshape(n_t, n_m, -1)
        if x2.shape[-1] == 1:
            x2 = np.squeeze(x2, axis=-1)
        else:
            x2 = x2.mean(axis=-1)
        scatter_data = x2.T  # (n_models, n_cell_types)
