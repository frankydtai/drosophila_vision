...
class TypeSpecificSNN(nn.Module):
    ...
    def forward(self, vis_feat, proprio, state=None):
        ...
        turn_scale = 3.0 + turn_abs * 8.0
        base_speed = torch.where(
            turn_abs > 0.15,
            torch.tensor(0.3),
            0.6 + total_dn * 0.4
        )
        left_leg  = torch.clamp(base_speed - turn_signal*turn_scale, 0.1, 1.5)
        right_leg = torch.clamp(base_speed + turn_signal*turn_scale, 0.1, 1.5)
...
def extract_visual_features_12d(vision, fly_pos, fly_heading,
    ...
    features = np.array([
        # ★ 使用复眼暗区差作为方向信号（果蝇B已放大3倍）★
        np.clip(dark_diff / MAX_DARK_DIFF, -1, 1),        # [0] 复眼暗区差
        np.clip(1.0 / (dist + 1.0), 0, 1),                # [1] target_dist_inv
        target_size,                                         # [2] target_size
        float(np.sign(dark_diff)),                          # [3] 暗区方向符号
        ...
        np.clip((left_dark+right_dark)/100.0, 0, 1),       # [9] 总暗区
