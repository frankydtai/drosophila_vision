...
class ConnectomeNetwork(nn.Module):
    ...
    synapse_model : str or BaseSynapse, optional
        Synapse model (``'simple'``, ``'tanh_rate'``, ``'tanh_conductance'``, 
        ``'ionotropic'``). Default ``'simple'`` uses weight-based synapses.
    dt : float
    ...
    learn_synapse_params : bool
        Whether synapse model parameters are trainable. Default True.
    ...
    def __init__(
        ...
        synapse_model: str | BaseSynapse | None = "simple",
        dt: float = 1.0,
        learn_weights: bool = True,
        learn_time_const: bool = True,
        learn_synapse_params: bool = True,
        ...
        # Synapse model
        self.synapse_model: Optional[BaseSynapse] = self._build_synapse_model(
            synapse_model, self.n_edges, learn_synapse_params
        )
        self.use_synapse_model = self.synapse_model is not None

    # ------------------------------------------------------------------
    # Synapse model builder
    # ------------------------------------------------------------------
    def _build_synapse_model(
        self,
        synapse_model: str | BaseSynapse | None,
        n_edges: int,
        learn_params: bool,
    ) -> Optional[BaseSynapse]:
        """Build synapse model from string or instance."""
        if synapse_model is None or synapse_model == "simple":
            return None  # Use simple weight-based synapses
        
        if isinstance(synapse_model, BaseSynapse):
        ...
        if synapse_model not in _registry:
            raise ValueError(
                f"Unknown synapse model: {synapse_model}. "
                f"Choose from: {['simple'] + list(_registry)}"
            )
        return _registry[synapse_model](n_edges, learn_params=learn_params)
    ...
    def forward(
        ...
        # Initialize synapse states if using synapse model
        synapse_states = None
        if self.use_synapse_model and hasattr(self.synapse_model, 'init_states'):
            synapse_states = self.synapse_model.init_states(B, device=x.device)
        ...
        for t in range(T):
            ...
            # Build per-step params with batch-broadcasted weight and indices
            step_params = dict(params)
            step_params["weight"]  = w
            step_params["pre_idx"] = self.pre_idx
            
            # If using synapse model, compute synaptic currents
            if self.use_synapse_model:
                # Get pre- and post-synaptic voltages
                # Assume first state variable is voltage
                primary_key = next(iter(state))
                voltages = state[primary_key]  # (B, N)
                
                # Gather voltages at synapses
                pre_voltage = voltages[:, self.pre_idx]   # (B, E)
                post_voltage = voltages[:, self.post_idx]  # (B, E)
                
                # Update synapse states (if stateful)
                if synapse_states is not None:
                    synapse_states = self.synapse_model.update_states(
                        synapse_states, dt, pre_voltage, post_voltage, None
                    )
                
                # Compute synaptic currents
                syn_currents = self.synapse_model.compute_current(
                    synapse_states or {}, pre_voltage, post_voltage, None
                )  # (B, E)
                
                # Apply weights and aggregate to post-synaptic neurons
                weighted_currents = w * syn_currents
                step_params["syn_input"] = target_sum(weighted_currents)  # (B, N)

            state = self.dynamics.step(state, step_params, x_t, dt, target_sum)

            # Primary observable: depends on dynamics type
            # For LIF: use 'z' (spikes) if available, else first state variable
            # For Voltage/HH: use first state variable (activity/v)
            if 'z' in state and isinstance(self.dynamics, type(self.dynamics)) and \
               self.dynamics.__class__.__name__ == 'LIFModel':
                activities.append(state['z'])  # (B, N) - spikes for LIF
            else:
                primary_key = next(iter(state))
    ...
    def extra_repr(self) -> str:
        synapse_info = (
            f"synapse={self.synapse_model.__class__.__name__}"
            if self.use_synapse_model
            else "synapse=simple"
        )
        return (
