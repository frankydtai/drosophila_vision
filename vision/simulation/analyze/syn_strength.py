...
Examples
--------
  # from simulation/ with the project .venv
  ../.venv/bin/python 7_analyze/3_syn_strength.py T4a --post
  ../.venv/bin/python 7_analyze/3_syn_strength.py Mi1 --x 0 --y 1
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if opts.get("model", "borst") not in ("borst", "hp_lp"):
        raise SystemExit(
            f"syn_strength requires borst/hp_lp model, got {opts.get('model')!r}"
        )
