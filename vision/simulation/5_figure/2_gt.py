"""Model-gt cell selection and gt cubes from session + task."""
...
def fit_gt_cubes(
    *, contrasts=("bright",), t_onset=None, n_t=None, ms_spot=None,
    ...
    ``filter==\"ca\"`` → Arenz digitized ImpR (same as train ``build_spot_gt``).
    """
    out = {}
...
def spot_gt_cubes(
    ...
    *,
    ...
    ``filter`` defaults to ``session.train_opts['filter']`` (train GT kind).
    """
