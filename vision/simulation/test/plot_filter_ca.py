...
Default ``v_th_ca`` / ``a_ca`` / ``tau_ca`` from ``PARAM_BOXES`` init;
...
TAU_CA = float(PARAM_BOXES["tau_ca"]["init"])
A_CA = float(PARAM_BOXES["a_ca"]["init"])
V_TH_CA = float(PARAM_BOXES["v_th_ca"]["init"])
IMPR_OFFSET = 0
