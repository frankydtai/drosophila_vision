...
The CSV holds **16 traces** per pathway panel: 2 contrasts × 2 widths × 2 directions. Subtype-specific tables below map each `(eye, stimulus)` to one of those keys via `t4_t5_dsi.py` (`fig1_key_for_stimulus` → `T4_PC_w1_PD` style ids). Training loads the same data as [`1ci_digitized.npz`](../../figure_digitization/gruntman/1ci_digitized.npz) (`FIG1_CI_NPZ` in `task.moving_bar.gt`).
