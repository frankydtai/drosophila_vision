...
LP-only (membrane low-pass):
    ...
    τ_m dV/dt = -(V - V_rest) + G S_in(t)

HP + LP (slow average a, high-pass drive into membrane):

    τ_HP da/dt = S_in - a
    S_HP = S_in - a
    τ_m dV/dt = -(V - V_rest) + G S_HP
...
Usage (from ``SimulationCode/``):
    ...
    ../.venv/bin/python test/plot_lp_hp_membrane.py
    ../.venv/bin/python test/plot_lp_hp_membrane.py --show
    ../.venv/bin/python test/plot_lp_hp_membrane.py --gain-list 0.5,1,2 --tau-hp-list 80,200,500 --pulse-list 50,100,500
"""
...
DEFAULT_SAVE = os.path.join(HERE, "lp_hp_membrane.png")
DEFAULT_GAIN_LIST = "0.5,1,2"
DEFAULT_TAU_HP_LIST = "80,200,500"
...
def membrane_lp(s_in: np.ndarray, *, tau_m_samples: float, v_rest: float, gain: float) -> np.ndarray:
    """V = V_rest + G * lowpass(S_in)."""
    return v_rest + gain * bs.lowpass(np.asarray(s_in, dtype=np.float64), tau_m_samples)
...
def membrane_hp_lp(
    ...
    *,
    ...
    if tau_hp_samples < 1.0:
        ...
        s_hp = x - a
    v = v_rest + gain * bs.lowpass(s_hp, tau_m_samples)
    return v, a, s_hp
...
def _fill_column(
    ...
    *,
    ...
    v_lp = membrane_lp(s_in, tau_m_samples=tau_m, v_rest=v_rest, gain=gain)
...
def plot_lp_hp(
    ...
    *,
    ...
    t_s = t_ms / 1000.0
    tau_m = tau_ms_to_samples(tau_m_ms, dt_ms)
    ...
    param_line = (
        rf"pulse={pulse_ms:g} ms, $\tau_m$={tau_m_ms:g} ms, "
...
def parse_args(argv=None):
    ...
    p.add_argument("--pulse-ms", type=float, default=500.0)
    ...
    p.add_argument("--tau-m", type=float, default=50.0, help="membrane tau [ms]")
